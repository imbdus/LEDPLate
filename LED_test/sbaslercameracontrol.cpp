...
void SBaslerCameraControl::setGain(double gain)
{
...
long SBaslerCameraControl::GrabImageOnlyOne(QImage &image,int timeout)
{
    try {
        if (m_basler.IsGrabbing()) {
            m_basler.StopGrabbing();
...
//        qDebug() <<"enter one image"<<GrabImage(image, 500);
        while (!(GrabImage(image, 500)==0));
        if (m_basler.IsGrabbing()) {
            m_basler.StopGrabbing();
        }
...
//        OutputDebugString(LPCWSTR(e.GetDescription()));
        qDebug() << "Acquire Error:" << QString(e.GetDescription());
        return -2;
    }
